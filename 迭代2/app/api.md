...
| `selections` | `selection[]` | 否 | 选择禁用的工具 |
...
```json
{
    "data": "Generated By Agent, using for testing",
    "question_id": 1
}
```
...
- 功能说明：获取行程下的所有记录的内容。
- 接口地址: `Get /api/record/{schedule_id}`
- 请求头
  - `Content-Type: application/json`
  - `Authorization: Bearer <token>`
...
```json
{
    ...
    "data": {
        "records": [
            ...
            {
                "id": 1,
                "data": "Generated By Agent, using for testing2",
                "question_id": 3
            }
        ],
        "currentPlan": {
