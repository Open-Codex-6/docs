...
#table(
  ...
  table.header[*包*][*模块*][*类/函数*][*描述与作用*],

  table.cell(rowspan: 16)[`ragas`],

  table.cell(rowspan: 2)[`evaluation`],
  [`evaluate()`], [主评估同步入口，接收测试数据集和指标，解析样本并调度 Executor 启动流水线返回结果。],
  [`aevaluate()`], [异步评估入口函数，支持协程环境下的高并发评测，由于不阻塞特别适用于长链路的 Agent 多轮对话。],
...
#table(
  ...
  table.cell(rowspan: 2)[`metrics.decorator`],
  [`@metric`], [快捷指标装饰器方法，支持在纯函数上挂接将直接解析成具体的 SimpleBaseMetric 实例化对象。],
