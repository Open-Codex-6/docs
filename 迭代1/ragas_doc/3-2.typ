...
#table(
  ...
  [`TestsetGenerator`], [测试集生成入口类，协调文档转换、知识图构建、查询分布采样与最终 Testset 产出。],
  ...
  [`SummaryExtractor`], [LLM 摘要提取器，生成节点摘要，服务于主题归纳与后续关系构建。],
...
#table(
  ...
  [`LiteLLMAdapter`], [LiteLLM 结构化输出适配器，用统一接口接入多家模型服务。],

  [`llms.litellm_llm`],
  [`LiteLLMStructuredLLM`], [LiteLLM 的结构化 LLM 实现，直接提供 `response_model` 驱动的生成能力。],
  ...
  [`OpenAIEmbeddings`], [OpenAI 向量模型实现，常用于语义检索和关系构建。],
  ...
  [`LiteLLMEmbeddings`], [LiteLLM embedding 统一接入层，便于跨提供方切换模型。],
)
...
#table(
  ...
  [`BasePrompt`], [提示系统抽象基类，约束生成接口与序列化能力，是所有 prompt 组件的共同父类。],
  [`StringPrompt`], [最简字符串提示模板，适用于轻量任务和快速格式化输出。],

  [`prompt.mixin`],
  [`PromptMixin`], [为合成器、提取器提供 prompt 注册、覆写和配置能力的混入类。],
  ...
  [`DynamicFewShotPrompt`], [动态 `few-shot` 提示类，按输入实时检索最相关示例以提升生成质量。],

  table.cell(rowspan: 2)[
    `prompt.`\
    `multi_modal_prompt`
  ],
  [`ImageTextPrompt`], [多模态提示类，组合图片与文本上下文完成视觉和文本联合推理。],
  [`ImageTextPromptValue`], [多模态提示值对象，封装可发送到底层模型的图文消息结构。],

  [
    `prompt.metrics.`\
    `base_prompt`
  ],
  [`BasePrompt`], [指标提示基类，专用于 `metric` 评分链路，支持多语言模板与结构化输出。],
)